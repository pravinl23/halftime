...
from extract_segment import extract_segment, parse_time
...
def process_video(
    ...
    try:
        ...
        print("-" * 40)
        # Convert times to floats for insert_segment
        start_float = parse_time(start_time) if isinstance(start_time, str) else float(start_time)
        end_float = parse_time(end_time) if isinstance(end_time, str) else float(end_time)
        insert_segment(input_path, edited_path, start_float, end_float, output_path)
