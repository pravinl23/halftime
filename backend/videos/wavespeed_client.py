...
class WaveSpeedClient:
    ...
    def _load_prompt_template(self) -> str:
        ...
        if os.path.exists(PROMPT_TEMPLATE_PATH):
            ...
            return "The video continues smoothly. A {product_name} by {company} appears naturally in the scene."
    ...
    def build_prompt(
        ...
        """
        Build a SIMPLE prompt for video generation.
        
        Wan 2.5 works best with short, visual descriptions.
        Complex context confuses it.
        """
        company = product_info.get('company', 'the brand')
        product_name = product_info.get('product', 'the product')
        product_category = product_info.get('category', 'product')
        
        # Simple, direct prompt for video generation
